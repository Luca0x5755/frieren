...
10. 寫入 `d:\frieren\novel\S{季}_{季別標題}\S{季}E{章}_{中文意譯暱稱}.md` 後，**同步在該季 `99_S{季}_心像真名索引.md` 補入該章新登場技術列**；回報路徑與重點摘要
...
10. 寫入 `d:\frieren\novel\S{季}_{季別標題}\S{季}E{章}_{試煉名}.md` 後，**在該季 `99_S{季}_心像真名索引.md` 對應條目備註欄標註「整合自 S{x}E{y}」（不新增條目）**；回報「整合技術清單」與該季 `00_四季時間軸總覽.md` 對照無誤
